...
namespace uppid
{
    ...
    // void DoublePrf::recvDDH(
    //     std::vector<__m128i>& input, 
    //     std::vector<__m128i>& UID, 
    //     Socket& chl)
    // {
    //     co_await chl.send(input.size());
    //     auto dhKeyByte = mDdh->dhKey.ToByteVector(BN_BYTE_LEN);
    //     auto myPRF = DDHOPRF::Evaluate(mDdh->pp, dhKeyByte, input, input.size());

    //     BigInt r = GenRandomBigIntLessThan(order); // pick a mask

    //     std::vector<ECPoint> maskedInput(input.size()); 
    //     for(size_t i = 0; i < input.size(); i++){
    //         maskedInput[i] = Hash::BlockToECPoint(input[i]) * r; // H(x_i)^r
    //     } 
    //     io.SendECPoints(maskedInput.data(), input.size());

    //     std::vector<ECPoint> doublyMaskedInput(input.size());
    //     io.ReceiveECPoints(doublyMaskedInput.data(), input.size()); // receive F_k(mask_x_i) from Server

    //     BigInt r_inverse = r.ModInverse(order); 
    //     std::vector<ECPoint> oprfInput(input.size());
    //     std::vector<std::vector<uint8_t>> oprf(input.size(), std::vector<uint8_t> (HASH_OUTPUT_LEN, 0)); 
    //     for (size_t i = 0; i < input.size(); i++){
    //         oprfInput[i] = doublyMaskedInput[i] * r_inverse; 
    //         oprf[i] = Hash::ECPointToBytes(oprfInput[i]); 
    //     }
    ...
    // void DoublePrf::sendDDH(Socket& chl)
    // {
    //     u64 theirSize;
    //     io.ReceiveInteger(theirSize);

    //     std::vector<ECPoint> maskedValue(theirSize); // H(x_i)^r, x_i: their input
    //     io.ReceiveECPoints(maskedValue.data(), theirSize);

    //     std::vector<ECPoint> doublyMaskedValue(theirSize); // H(x_i)^(rk)
    //     for (size_t i = 0; i < theirSize; i++){ 
    //         doublyMaskedValue[i] = maskedValue[i] * mDdh->dhKey;
    //     }

    //     io.SendECPoints(doublyMaskedValue.data(), theirSize);
    // }

    // Proto DoublePrf::recvDDH(
    //     oc::span<oc::block> input, 
    //     std::vector<oc::block>& UID, 
    //     Socket& chl)
    // {
    ...
    //     }
    //     auto dhKeyByte = mDdh->dhKey.ToByteVector(BN_BYTE_LEN);
    //     auto myPRF = DDHOPRF::Evaluate(mDdh->pp, dhKeyByte, input_m128, input.size());
    ...
    // Proto DoublePrf::sendDDH(Socket& chl)
    // {
    //     u64 theirSize;
    //     co_await chl.recv(theirSize);
